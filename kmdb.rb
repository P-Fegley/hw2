...
# run in terminal> rails generate model Movie
# run in terminal> rails generate model Studio
# run in terminal> rails generate model Actor
# run in terminal> rails generate model Role
# Edit schema in db:migrate files
...
new_role = Role.new
new_role["movie_id"] = dark_knight["id"]
new_role["actor_id"] = mgyllenhaal["id"]
new_role["character_name"] = "Rachel Dawes"
new_role.save
...
# I don't think there's a good way to get it in nice column form, at least not that we were taught.
...
for line in Role.all
    movie = Movie.find_by({ "id" => line["movie_id"] })["title"]
    actor = Actor.find_by({ "id" => line["actor_id"] })["name"]
